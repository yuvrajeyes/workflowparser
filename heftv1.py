...
class HEFTExample():
    ...
    def create_VM(userId: int, vms: int) -> List[CondorVM]:
        ...
        currentTimeMillis = int(time.time() * 1000)  # Get current time in milliseconds
        bwRandom: random.Random = random.Random(currentTimeMillis)

        for i in range(vms):
            ratio: float = 0.5 # bwRandom.random()  # Generate a random float between 0.0 and 1.0
    ...
    def main() -> None:
        ...
        dax_file = os.path.abspath(daxPath)
        ...
        vmlist0: List[CustomVM] = HEFTExample.create_VM(wfEngine.get_scheduler_id(0), Parameters.getVmNum())
